...
public class CleaningLogDAO
{
  ...
  public ArrayList<SolarPanel> getAllCleaningLogByTime(String startDate, String endDate) throws SQLException
  {
    ...
    {
      PreparedStatement statement = connection.prepareStatement(
          "SELECT sp.serial_number, sp.cleaning_time, sp.location, sp.type FROM solarex.cleaning_log JOIN solarex.solar_panel sp on sp.serial_number = cleaning_log.serial_number WHERE sp.cleaning_time BETWEEN ? AND ?");
      ...
      while (resultSet.next())
      {
        ...
        SolarPanel solarPanel = new SolarPanel(serialNo, cleaningDate, location, type);
  ...
  public ArrayList<SolarPanel> getCleaningTimes() throws SQLException
  {
    try (Connection connection = getConnection())
    {
      PreparedStatement statement = connection.prepareStatement("SELECT serial_number, location, cleaning_time, type FROM solarex.solar_panel");
      ResultSet resultSet = statement.executeQuery();
      ArrayList<SolarPanel> results = new ArrayList<>();
      while(resultSet.next())
      {
  ...
  public void addCleaningTime(double sn, String cleaningDate) throws SQLException
  {
    try(Connection connection = getConnection())
    {
      PreparedStatement statement = connection.prepareStatement("UPDATE solarex.solar_panel SET cleaning_time = ? WHERE serial_number = ?");
